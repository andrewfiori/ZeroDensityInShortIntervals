...
# This file was *autogenerated* from the file Code/indep_arb_hcompact.sage
from sage.all_cmdline import *   # import sage library

_sage_const_100 = Integer(100); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_14 = Integer(14); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_90 = Integer(90); _sage_const_27 = Integer(27); _sage_const_164 = Integer(164); _sage_const_7 = Integer(7); _sage_const_6 = Integer(6); _sage_const_16 = Integer(16); _sage_const_9 = Integer(9); _sage_const_41 = Integer(41); _sage_const_64 = Integer(64); _sage_const_12 = Integer(12)
import time

PREC = _sage_const_100 
CB = ComplexBallField(PREC)
RB = RealBallField(PREC)
E_RB = RB(_sage_const_1 ).exp()
...
def GPL(Q, s):
    return (CB(Q + s).log() + CB(Q + _sage_const_2  - s).log()) / _sage_const_2 
...
def f_enclosure(sball, tball):
    ...
    S = CB(sball, tball)
    fm = (m - _sage_const_1 ) * m.zeta()
    f1m = m.zeta() + (m - _sage_const_1 ) * zetaderiv(_sage_const_1 , m)
    f2S = _sage_const_2  * zetaderiv(_sage_const_1 , S) + (S - _sage_const_1 ) * zetaderiv(_sage_const_2 , S)
    d = S - m
    return fm + f1m * d + f2S * d * d / _sage_const_2 
...
class Case:
    ...
    def inf_lower(self, tball, ntau=_sage_const_8 ):
        out = []
        for kind, G in self.Gs:
            if kind == "linG":
                ...
                best = None
                for j in range(ntau):
                    tau = ball(self.a + (self.b - self.a) * j / ntau, self.a + (self.b - self.a) * (j + _sage_const_1 ) / ntau)
    ...
    def check_box(self, s0, s1, t0, t1, depth, stats, maxdepth=_sage_const_14 ):
        stats["boxes"] += _sage_const_1 
        tball = ball(t0, t1)
        ...
        ok = True
        if (s1 - s0) / (self.b - self.a) > (t1 - t0) / max(self.T0, _sage_const_1 ):
            sm = (s0 + s1) / _sage_const_2 
            ok &= self.check_box(s0, sm, t0, t1, depth + _sage_const_1 , stats, maxdepth)
            ok &= self.check_box(sm, s1, t0, t1, depth + _sage_const_1 , stats, maxdepth)
        else:
            tm = (t0 + t1) / _sage_const_2 
            ok &= self.check_box(s0, s1, t0, tm, depth + _sage_const_1 , stats, maxdepth)
            ok &= self.check_box(s0, s1, tm, t1, depth + _sage_const_1 , stats, maxdepth)
        return ok

    def run(self, nt0=None):
        stats = {"boxes": _sage_const_0 , "worst": RR(_sage_const_0 ), "failed": []}
        t_start = time.time()
        # initial t-partition of [0,T0] into pieces of width ~1/8, sigma whole
        T0 = self.T0
        n = nt0 or int(ceil(RR(T0) * _sage_const_8 ))
        ok = True
        for i in range(n):
            t0 = T0 * i / n
            t1 = T0 * (i + _sage_const_1 ) / n
            ok &= self.check_box(self.a, self.b, t0, t1, _sage_const_0 , stats)
        el = time.time() - t_start
        print(f"  {self.name:8s}: {'CERTIFIED' if ok else 'FAILED'}  boxes={stats['boxes']:5d}  "
              f"worst certified U/L={stats['worst']:.4f}  ({el:.1f}s)")
        for f in stats["failed"][:_sage_const_5 ]:
            print(f"      unresolved box sigma[{RR(f[_sage_const_0 ]):.4f},{RR(f[_sage_const_1 ]):.4f}] t[{RR(f[_sage_const_2 ]):.4f},{RR(f[_sage_const_3 ]):.4f}]: U={f[_sage_const_4 ]:.4f} L={f[_sage_const_5 ]:.4f}")
        return ok


def interpG(A, B, p, q, Q):
    A, B = RB(A), RB(B)
    C0 = A ** (RB(_sage_const_1 ) / (p + _sage_const_1 ))
    C1 = (B / C0) ** (RB(_sage_const_1 ) / q)
    return [("linG", lambda s: CB(C0) * s), ("linGRefl", lambda s: CB(C1) * (_sage_const_1  - s)),
            ("GPL", lambda s: GPL(Q, s))]


def interpG1c(B, c, d, q, Q):
    B, c, d = RB(B), RB(c), RB(d)
    C2 = B ** (RB(_sage_const_1 ) / q)
    return [("linG", lambda s: s), ("G1shift", lambda s: CB(c) * GPL(Q, s + _sage_const_1 ) + CB(d)),
            ("linGRefl", lambda s: CB(C2) * (_sage_const_1  - s)), ("GPL", lambda s: GPL(Q, s))]


def sigma_k(k):
    return _sage_const_1  - QQ(k) / (_sage_const_2  ** k - _sage_const_2 )


print("=" * _sage_const_90 )
print("Certified box checks (arb, prec 100 bits). t >= 0 only (conjugation symmetry).")
print("=" * _sage_const_90 )
T0_big = _sage_const_4  * E_RB + _sage_const_1 
p, q = QQ(_sage_const_27 ) / _sage_const_164 , QQ(_sage_const_1 ) / _sage_const_14 
results = {}
results["1a"] = Case("1a", QQ(_sage_const_1 ) / _sage_const_2 , QQ(_sage_const_5 ) / _sage_const_7 , T0_big, interpG(RB("66.7"), RB("1.546"), p, q, _sage_const_4  * E_RB),
                     [p + _sage_const_1 , _sage_const_0 , _sage_const_0 ], [_sage_const_1 , q, _sage_const_1 ]).run()
p = QQ(_sage_const_1 ) / _sage_const_6 
results["1b"] = Case("1b", QQ(_sage_const_1 ) / _sage_const_2 , QQ(_sage_const_5 ) / _sage_const_7 , RB(_sage_const_3 ), interpG(RB("0.611"), RB("1.546"), p, q, RB(_sage_const_16 )),
                     [p + _sage_const_1 , _sage_const_0 , _sage_const_1 ], [_sage_const_1 , q, _sage_const_1 ]).run()
results["1c"] = Case("1c", QQ(_sage_const_1 ) / _sage_const_2 , QQ(_sage_const_5 ) / _sage_const_7 , T0_big,
                     interpG1c(RB("1.546"), RB("0.470795"), RB("4.04972"), q, _sage_const_4  * E_RB),
                     [p + _sage_const_1 , _sage_const_1 , _sage_const_0 , _sage_const_0 ], [_sage_const_1 , _sage_const_0 , q, _sage_const_1 ]).run()
for k in range(_sage_const_4 , _sage_const_8 ):
    a, b = sigma_k(k), sigma_k(k + _sage_const_1 )
    pk, qk = QQ(_sage_const_1 ) / (_sage_const_2  ** k - _sage_const_2 ), QQ(_sage_const_1 ) / (_sage_const_2  ** (k + _sage_const_1 ) - _sage_const_2 )
    results[f"k={k}"] = Case(f"2:k={k}", a, b, RB(_sage_const_3 ), interpG(RB("1.546"), RB("1.546"), pk, qk, E_RB),
                             [pk + _sage_const_1 , _sage_const_0 , _sage_const_1 ], [_sage_const_1 , qk, _sage_const_1 ]).run()
...
print("=" * _sage_const_90 )
A = RB("1.546")
s8, s9 = sigma_k(_sage_const_8 ), sigma_k(_sage_const_9 )
p8, q8 = QQ(_sage_const_1 ) / (_sage_const_2  ** _sage_const_8  - _sage_const_2 ), QQ(_sage_const_1 ) / (_sage_const_2  ** _sage_const_9  - _sage_const_2 )
mono_ok = True
prev = None
for k in range(_sage_const_8 , _sage_const_41 ):
    bk = sigma_k(k + _sage_const_1 )
    qk = QQ(_sage_const_1 ) / (_sage_const_2  ** (k + _sage_const_1 ) - _sage_const_2 )
    val = RR(qk) * abs(RR(log(RR(_sage_const_1  - bk))))
    if prev is not None and val > prev:
        mono_ok = False
    prev = val
print(f"  q_k |log(1-sigma_(k+1))| decreasing for k=8..40: {mono_ok}   -> (1-sigma_(k+1))^q_k >= (1-sigma_9)^q_8 = {RR((_sage_const_1 -s9)**q8):.6f}")
term1_low = RB(_sage_const_1  - s9) ** RB(q8)

# M = max_{|w-1| = 1/4} |zeta(w) - 1/(w-1)| certified on arcs
M = RR(_sage_const_0 )
NARC = _sage_const_64 
for j in range(NARC):
    th = ball(_sage_const_2  * RB.pi() * j / NARC, _sage_const_2  * RB.pi() * (j + _sage_const_1 ) / NARC)
    w = CB(_sage_const_1 ) + CB(RB(_sage_const_1 ) / _sage_const_4 ) * CB(th.cos(), th.sin())
    v = (w.zeta() - _sage_const_1  / (w - _sage_const_1 )).abs().upper()
    M = max(M, RR(v))
M = RB(M)
...
def tail_R_low(tball):
    base = A ** (RB(_sage_const_1 ) / (_sage_const_1  + p8)) * (RB(s8) ** _sage_const_2  + tball ** _sage_const_2 ).sqrt()
    f0 = RB(min(base.lower(), (base ** RB(_sage_const_1  + p8)).lower()))
    best = None
    for j in range(_sage_const_16 ):
        tau = ball(s8 + (_sage_const_1  - s8) * j / _sage_const_16 , s8 + (_sage_const_1  - s8) * (j + _sage_const_1 ) / _sage_const_16 )
...
def tail_lhs_upper(s0, s1, t0, t1):
    ...
    rad = RealField(PREC, rnd='RNDU')(max((_sage_const_1  - s0) ** _sage_const_2 , (_sage_const_1  - s1) ** _sage_const_2 ) + max(t0 ** _sage_const_2 , t1 ** _sage_const_2 )).sqrt()
    if rad <= RR(_sage_const_1 ) / _sage_const_4 :
        return (RB(_sage_const_1 ) + RB(rad) * M).upper()
    return f_enclosure(sball, tball).abs().upper()


def tail_check(s0, s1, t0, t1, depth, stats, maxdepth=_sage_const_12 ):
    stats["boxes"] += _sage_const_1 
    ...
    if depth >= maxdepth:
        ...
        return False
    ok = True
    if (s1 - s0) / (_sage_const_1  - s8) > (t1 - t0) / _sage_const_3 :
        sm = (s0 + s1) / _sage_const_2 
        ok &= tail_check(s0, sm, t0, t1, depth + _sage_const_1 , stats, maxdepth)
        ok &= tail_check(sm, s1, t0, t1, depth + _sage_const_1 , stats, maxdepth)
    else:
        tm = (t0 + t1) / _sage_const_2 
        ok &= tail_check(s0, s1, t0, tm, depth + _sage_const_1 , stats, maxdepth)
        ok &= tail_check(s0, s1, tm, t1, depth + _sage_const_1 , stats, maxdepth)
    return ok
...
for i in range(_sage_const_12 ):
    ok &= tail_check(s8, QQ(_sage_const_1 ), QQ(i) / _sage_const_4 , QQ(i + _sage_const_1 ) / _sage_const_4 , _sage_const_0 , stats)
